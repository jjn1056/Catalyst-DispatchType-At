package Catalyst::DispatchType::At;

use Moose;
extends 'Catalyst::DispatchType';

use Text::SimpleTable;
use Catalyst::ActionChainNext;
use Catalyst::Utils;
use URI;
use Scalar::Util ();
...
Catalyst::DispatchType::At - Path Part DispatchType
...
This is a fork of L<Catalyst::DispatchType::Chained> since I can't get anyone to
agree to make changes to it.  Its works the same just hase the new <next> acti\on
feature.  See <Catalyst::ActionChainNext> for more info on that.
...
sub match {
    ...
    my $action = Catalyst::ActionChainNext->from_chain($chain);
...
sub register {
    my ( $self, $c, $action ) = @_;
    my @chained_attr = @{ $action->attributes->{At} || [] };
...
sub uri_for_action {
    ...
    return undef unless ($action->attributes->{At}
...
sub expand_action {
    ...
    return unless $action->attributes && $action->attributes->{At};
    ...
    while ($curr) {
        push @chain, $curr;
        my $parent = $curr->attributes->{At}->[0];
        $curr = $self->_actions->{$parent};
    }

    return Catalyst::ActionChainNext->from_chain([reverse @chain]);
}
